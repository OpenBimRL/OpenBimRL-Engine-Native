...
namespace OpenBimRL::Native::Properties { extern PropertySets g_propertySets; extern QuantitySets g_quantitySets; }
...
void handlePSetImpl(const typename Schema::IfcPropertySet* set) {
    if (!set || !set->Name() || set->Name()->empty()) return;
    ...
    for (const auto prop : (*props)) {
        ...
        OpenBimRL::Native::Properties::g_propertySets[setName][propName] = propVal;
    }
}
...
void handleQSetImpl(const typename Schema::IfcQuantitySet* set) {
    ...
    for (const auto quantity : (*quantities)) {
        ...
        OpenBimRL::Native::Properties::g_quantitySets[name.value()][quantity->Name()] =
            getQuantityValueImpl<Schema>(quantity);
    }
}
